...
class Ninja:
    ...
    @classmethod
    def get_all(cls):
        query = "SELECT * from ninjas LEFT JOIN users on ninjas.user_id = users.id;"
    ...
    @classmethod
    def save(cls, data):
        query = 'INSERT INTO ninjas (firstName, last_name, age, user_id ) VALUES ( %(first_name)s, %(last_name)s, %(age)s , %(user_id)s);'
    ...
    @classmethod
    def get_userNinjas(cls, data):
        query = "SELECT * from ninjas LEFT JOIN users on ninjas.user_id = %(user_id)s;"
        results = connectToMySQL('dojos_and_ninjas').query_db(query, data)
        ninjas = []
        for row in results:
            print(row)
